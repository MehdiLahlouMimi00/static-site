function Bubbles(container, self, options) {
  ...
  interactionsSaveCommit = function() {
    ...
    // Constructing the object with question-answer pairs
    var conversationObject = {};
    interactionsHistory.forEach(function(interaction) {
      // Extracting question and answer from the interaction
      var question = interaction.say.replace('<span class="bubble-content">', '').replace('</span>', '');
      var answer = interaction.reply.replace('<span class="bubble-content">', '').replace('</span>', '');
  
      // Adding the question-answer pair to the object
      conversationObject[question] = answer;
    });
  
    console.log(conversationObject); // Display in console
    alert(JSON.stringify(conversationObject)); // Display in alert as JSON string
    localStorage.setItem(interactionsLS, JSON.stringify(conversationObject));
  }
  // set up the stage
